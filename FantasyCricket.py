...
class Ui_MainWindow(object):
    ...
    def open_team(self):
        ...
        self.team, ok = QtWidgets.QInputDialog.getItem(
            MainWindow, "Open", "Choose A Team", self.teams, 0, False)
        if ok == True:
            ...
            self.Pointused = 0
            self.AllTeams = []
            self.WK_Button.clicked.connect(
                self.WK_players)  # ----------- WK Button
            self.BAT_Button.clicked.connect(
                self.BAT_players)  # --------BAT BUTTON
            self.BOW_Button.clicked.connect(
                self.BOW_players)  # =-------BOW BUTTON
            self.AR_Button.clicked.connect(
                self.AR_players)  # --------AR BUTTON
            ...
            for i in objcricket.fetchall():
                if i[1] == self.team:
                    ...
                    self.PointAvl_count.setText(
                        str(self.PointAvl-self.Pointused))
                ...
                if self.entername.text() not in self.teamlist:
                    for i in objcricket.fetchall():
                        if i[0] in self.playersSelectedList:
                            ...
                            cricket.commit()
                            self.k += 1
                    self.messagebox('Hurry', "%s Team Saved Successfully" % (
                        self.entername.text()))
                    self.PlayersList.clear()
                    self.SelectedPlayers.clear()
                    self.playersSelectedList = []
                    ...
                    self.messagebox('Sorry', '%s Team Already Exist.\n(If You Want to Modify Team, Delete it and Save Modified Team)' % (
                        self.entername.text()))
            else:
    ...
    def new_team(self):
        self.messagebox(
            'Welcome', '''Please Enter Team Name in the Team Name section\n(Choose 11 Players Precisely)''')
        self.PlayersList.clear()
        self.SelectedPlayers.clear()
        self.playersSelectedList = []
        self.count_BAT = 0
                ...
                for i in objcricket.fetchall():
                    if item.text() == i[0]:
                        if self.PointAvl >= i[2]:
                            if i[1] == 'BAT':
                                ...
                                if self.count_WK <= 1:
                                    self.WK_count.setText(
                                        '%s' % (self.count_WK))
                                    self.SelectedPlayers.addItem(item.text())
                                    self.playersSelectedList.append(
                                        item.text())
                                elif self.count_WK == 2:
                                    self.messagebox(
                                        'Warning', "You can't select more than 1 Wicket Keeper")
                                    break
                            elif i[1] == 'AR':
                                self.count_AR += 1
                            ...
                            self.messagebox(
                                'Warning', "You don't you have enough Points")
                            break
            else:
                self.messagebox(
                    'Warning', "You Can't select same Player Again")
        else:
    ...
    def setupUi(self, MainWindow):
        ...
        self.PlayersList.itemDoubleClicked.connect(
            self.removePlayersList)     # Connecting Method removePlayersList
        self.SelectedPlayers.itemDoubleClicked.connect(
            self.removeSelectedPlayers)  # Connecting Method SelectedPlayers
        ...
        self.actionEvaluate_Team.setIcon(
            QtGui.QIcon('Icons\\evaluateteam.png'))
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate(
            "MainWindow", "Cricket Fantasy Game"))
        self.BAT_label_2.setText(_translate(
            "MainWindow", "<img src='Icons\\Bat.png' width=25 height=25>Batsman(BAT)"))
        self.BOW_label_2.setText(_translate(
            "MainWindow", "<img src='Icons\\Ball.png' width=25 height=25>Bowlers(BOW)"))
        self.label_6.setText(_translate("MainWindow", "Your Selection"))
        self.teamname_label.setText(_translate("MainWindow", "Team Name :"))
        self.WK_label_2.setText(_translate(
            "MainWindow", "<img src='Icons\\Wicket.png' width=25 height=25>Wicket-keeper(WK)"))
        ...
        self.AR_label_2.setText(_translate(
            "MainWindow", "<img src='Icons\\Allrounder.png' width=25 height=25>AllRounders(AR)"))
        self.PtsAVL_label.setText(_translate(
            "MainWindow", "Points Available :"))
